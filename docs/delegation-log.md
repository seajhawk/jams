...
| 2026-09-12 | Temporal contrast evaluation | Codex subagent | gpt-5.6-luna | ✅ | Evaluated fixed 4–8 kHz peak/shoulder ratios across 27 crossed cases; max precision 25.70%, rejected for production. |
| 2026-09-12 | Audio onset optimization review | Codex subagent | gpt-6-astra | ✅ | Reviewed shared frame-energy computation; no regression found. Byte-equivalent outputs verified by primary. |
...
- **ROUTING CHANGE (Chris, 2026-07-17, superseded 2026-09-17):** Copilot's July 31 credit reset has long since happened, so the original bench is stale, but Copilot stays benched anyway for a different reason: Chris has $94.60 in Claude Code promo usage credits expiring 2026-09-19. **Temporary override (2026-09-17 to 2026-09-19 or until spent): Claude implements directly instead of delegating**, to burn the expiring credits before falling back to paid delegates. Codex is also currently out of credits (session/monthly exhaustion, not yet reset). Once the promo credits are spent or expired, resume normal priority (Copilot → Codex → aider/OpenRouter). **If Codex hits rate limits/exhaustion: STOP delegating and pause for Chris's decision — do not silently fall back.**
